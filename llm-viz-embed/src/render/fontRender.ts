...
function makeAssetUrl(basePath: string, path: string) {
    let base = basePath ? basePath.replace(/\/$/, '') : '';
    let cleanPath = path.replace(/^\//, '');
    return base ? `${base}/${cleanPath}` : `/${cleanPath}`;
}

export async function fetchFontAtlasData(basePath: string = ''): Promise<IFontAtlasData> {
    let imgEl = document.createElement('img');
    let imgP = new Promise<HTMLImageElement>((resolve, reject) => {
        imgEl.onload = () => resolve(imgEl);
        imgEl.onerror = () => reject();
    });
    imgEl.src = makeAssetUrl(basePath, 'fonts/font-atlas.png');

    let fontDefP = fetch(makeAssetUrl(basePath, 'fonts/font-def.json')).then(r => r.json());
