...
export function genModelViewMatrices(state: IProgramState, layout: IModelLayout, modelOffset: Vec3 = Vec3.zero) {
    ...
    const aspectScale = (window as any).LLM_VIZ_ASPECT_SCALE ?? 0.85;
    let persp = Mat4f.fromPersp(40, (state.render.size.x / state.render.size.y) * aspectScale, 100, 10000000);
    ...
    modelMtx[9] = -1.0;
    modelMtx[10] = 0.0;
    const modelScale = (window as any).LLM_VIZ_MODEL_SCALE ?? 0.92;
    modelMtx[0] *= modelScale;
    modelMtx[6] *= modelScale;
    modelMtx[9] *= modelScale;
    modelMtx[10] *= modelScale;
...
export function updateCamera(state: IProgramState, view: IRenderView) {
    const autoRotate = (window as any).LLM_VIZ_AUTO_ROTATE ?? true;
    if (autoRotate) {
        state.camera.angle.x += view.dt * 0.01;
        view.markDirty();
    }
