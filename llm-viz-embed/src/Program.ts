...
export function initProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IProgramState {
    ...
    let camera: ICamera = {
        ...
        center: prevState?.camera.center ?? new Vec3(-8.4, -90, -481.5),
        transition: {},
        modelMtx: new Mat4f(),
        viewMtx: new Mat4f(),
        lookAtMtx: new Mat4f(),
...
export function runProgram(view: IRenderView, state: IProgramState) {
    ...
    const modelOffset = new Vec3(0, 140, 0);
    genModelViewMatrices(state, state.layout!, modelOffset);
    ...
    for (let example of state.examples) {
        if (showModelUi && example.enabled && example.layout) {
